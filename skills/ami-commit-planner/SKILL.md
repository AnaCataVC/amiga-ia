...
## Workflow
1. **Analyze Working Tree & Local History:** Use git commands (`git status`, `git diff`, `git diff --cached`, and `git log @{u}..HEAD` or `git log -n 5 --oneline`) to identify all uncommitted modified/new/deleted files and review recently created unpushed commits.
...
5. **Request Approval:** Present the complete commit plan (including security audit confirmation and any proposed amend/squash operations) to the user in the chat. Ask for their approval and iterate if necessary. **NEVER** stage, commit, amend, or rebase anything until the user gives their explicit approval.
