...
8. **Atomic Version Control & UI Badges:** In this repository, adding or fusing skills/agents represents a new product capability and constitutes a **Minor Bump (`0.x.0`)**. Before publishing a new Release, the AI MUST proactively update package manager files (`package.json`) and any version badges on product/showcase pages (e.g., `index.html`) to the target version in a single atomic pre-tag commit (e.g., `chore(release): bump version to vX.Y.Z [skip ci]`).
9. **Git Cleanliness & Rebase Strategy:** Releases are 100% atomic pre-tag operations. GitHub Actions does not mutate `main` or generate ghost commits during release publishing. When preparing to push, ensure local branches are cleanly rebased against `origin/main` without relying on post-release pipeline commits.
...
14. **Release Notes Product Scope:** Release notes MUST strictly focus on core distributed product value (skills, agent orchestrators, hooks, security, package stability, and CLI tools). Internal repository housekeeping, internal documentation synchronization (e.g., updating README table counts, fixing internal docs links, guidelines maintenance), and promotional landing page edits (`index.html`) are NOT user-facing product features and MUST be strictly omitted from Release Notes.
...
20. **Zero Country Flags in Documentation & UI:** All documentation (`README.md`, `docs/`, `wiki/`), release notes, changelogs, table of contents, badges, and user interfaces created or maintained across repositories MUST NEVER include country flag emojis or flag graphics. Flags represent countries and political states, not languages, and degrade into broken gliphs on Windows. Use clean semantic text (`[English](#english) | [Español](#español)`), ISO codes (`EN`, `ES`), or unadorned headings (`## English`, `## Español`).
21. **Mandatory GitHub Actions CI/CD Verification:** Whenever an operation triggers a remote automated workflow (such as `git push`, `gh pr create`, `gh pr merge`, or `gh release create`), the AI MUST inspect active workflow runs (`gh run list --limit 3`) and monitor execution until completion (`gh run watch <run-id>`). The AI MUST verify that the workflow completed with `conclusion: success`. If a workflow run fails, the AI MUST immediately fetch the failed logs (`gh run view <run-id> --log-failed`), diagnose the failure (e.g. expired tokens/secrets, compilation defects), and alert the user. The AI MUST NEVER declare a release or push deployment completed successfully if its corresponding GitHub Actions workflow is failing.
22. **Internal Planning Reference Prevention:** All commit messages, Pull Request titles/descriptions, code docstrings, and code comments MUST describe changes, architecture, contracts, and intent strictly from a permanent domain and system perspective. The AI MUST NEVER include references to internal planning phases, execution stages, task steps, or decision options (e.g., 'Phase 1', 'Stage 2', 'Etapa 3', 'Option A', 'Opción B', 'Step 2 of plan') from internal planning dialogues, scratchpads, prompt plans, or checklists not exposed in the repository (and it is not necessary to expose them, only not reference them). Always explain WHAT changed and WHY from an architectural and domain perspective instead of referencing conversational task checklists or temporary development phases.
